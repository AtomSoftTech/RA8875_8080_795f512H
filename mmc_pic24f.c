...
//#define CS_LOW(SD)  SD_CS_LOW(SD)	/* MMC CS = L */
//#define CS_HIGH() _LATB15 = 1	/* MMC CS = H */
#define INS	PORTBbits.RB4//!(PORTB & (1<<11))	/* Card detected   (yes:true, no:false, default:true) */
#define WP	0	/* Write protected (yes:true, no:false, default:false) */

#define	FCLK_SLOW() SpiChnClose(2);    SpiInitDevice(2,SPI_SLOW,0)		/* Set slow clock (100k-400k) */
#define	FCLK_FAST() SpiChnClose(2);    SpiInitDevice(2,SPI_MED,0)			/* Set fast clock (depends on the CSD) */
...
#define RCVR_SPI_MULTI(dst,cnt) {UINT c=cnt/2; BYTE *p=dst; do {SpiChnPutC(2,0xFF);while(SpiChnIsBusy(2) > 0);*p++ = SpiChnGetC(2);SpiChnPutC(2,0xFF); while(SpiChnIsBusy(2) > 0);*p++ = SpiChnGetC(2);} while(--c);}
#define XMIT_SPI_MULTI(src,cnt) {UINT c=cnt/2;const BYTE *p=src; do {	SpiChnPutC(2,*p++);while(SpiChnIsBusy(2) > 0);SpiChnGetC(2); SpiChnPutC(2,*p++);while(SpiChnIsBusy(2) > 0);SpiChnGetC(2);} while(--c);}
/*-----------------------------------------------------------------------*/
/* Wait for card ready                                                   */
/*-----------------------------------------------------------------------*/
